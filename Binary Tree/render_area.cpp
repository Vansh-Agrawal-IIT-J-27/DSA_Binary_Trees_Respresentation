...
RenderArea::RenderArea(BinarySearchTree<int> *bst, QWidget *parent) : QWidget(parent), bst(),scale(1.0){
    this->bst = bst;
    this->scale = 1;
...
void RenderArea::mouseReleaseEvent(QMouseEvent *event){
    ...
    case Qt::LeftButton:
        if ( event->modifiers() & Qt::ControlModifier )
        {
            ...
            this->bst->deleteAtLocation(event->position().x(), event->position().y());
            this->repaint();
            break;
        }
